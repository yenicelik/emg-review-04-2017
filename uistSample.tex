...
We will present challenges that must be overcome to solve the cross-session problem, and show past approaches that tried to solve this problem. Little work has been done on cross-session sEMG measurements, so  (Martinez-Valdes et al. 2016) to find papers that included cross-session application, papers that included the "term", "days" or "weeks" were taken into consideration.
